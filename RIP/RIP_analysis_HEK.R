...
rm(list=ls())
setwd("/Users/gilbertgiri/Desktop/Dominguez_Lab/PCBP1/RIPs/HEK/")
...
assay<-sapply(sample,function(x){strsplit(x,"_|-")[[1]][2]})
condition<-sapply(sample,function(x){strsplit(x,"_|-")[[1]][1]})
batch<-rep(c(1,2),6)
sampleInfo<-as.data.frame(cbind(batch,sample,condition,assay))
sampleInfo

#Reading into DESEQ2
deseqdata <- DESeqDataSetFromTximport(txi.rsem, colData = sampleInfo, design = ~batch+assay+condition+assay:condition)
deseqdata

# pre-filtering- Averaging at 5 per sample
keep <- rowSums(counts(deseqdata)) >= 60  #23434
deseqdata <- deseqdata[keep,]

#checking the factor levels
deseqdata$condition<-relevel(deseqdata$condition,ref = "gfp")
deseqdata$assay<-relevel(deseqdata$assay,ref="input")

###Running DESEQ
deseqdata<-estimateSizeFactors(deseqdata)
deseqdata<-estimateDispersions(deseqdata)
deseqdata<-nbinomLRT(deseqdata,reduced=~batch+assay+condition)

###Looking at the variability between replicates
par(mfrow=c(1,1))
df <- as.data.frame(t(counts(deseqdata,normalize=T)))
df$Sample<-sapply(colnames(counts(deseqdata,normalize=T)),function(x){strsplit(x,"_|[.]")[[1]][3]})
pca_res <- prcomp(df[1:dim(df)[2]-1])
pca_res<-as.list(pca_res)
autoplot(pca_res,data=df,colour="Sample")

#Visualizing the effect of batch effect
vsd <- vst(deseqdata)
plotPCA(vsd, "batch")
...
plotPCA(vsd, c("assay","condition"))


### Comparing fold change in mutant vs wildtype
resultsNames(deseqdata)
res<-results(deseqdata,contrast = list(c("assayflag.conditionwt","assayflag.conditionmut")))
res
summary(res)
...
res_df$Gene_Symbol = mapIds(org.Hs.eg.db,
                            ...
                            keytype="ENSEMBL",
                            multiVals = "first")
write.table(res_df,"../RIP_DESeq_summary_HEK.txt",quote=F,row.names = F,col.names = T,sep="\t")
